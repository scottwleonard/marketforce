extends SceneTree
## Scene builder — run: timeout 60 godot --headless --script scenes/build_game_screen.gd

func _initialize() -> void:
	print("Generating: GameScreen")

	var root := Control.new()
	root.name = "GameScreen"
	root.set_anchors_preset(Control.PRESET_FULL_RECT)
	root.set_script(load("res://scripts/game_screen_controller.gd"))
	...
	if err != OK:
		...
		quit(1)
		return
	if not validate_packed_scene(packed, count, "res://scenes/game_screen.tscn"):
		quit(1)
		return

	err = ResourceSaver.save(packed, "res://scenes/game_screen.tscn")
	if err != OK:
	...
	print("Saved: res://scenes/game_screen.tscn")
	quit(0)
